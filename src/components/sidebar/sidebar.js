			...
			<div className="sidebar__content">
				<div className="sidebar__header">
					...
					<div className="sidebar__tab-heading">
						<button className={state.showColorPalette ? "active" : ""} onClick={(e) => handleButtonClick("SET_COLOR_PALETTE_VISIBILITY")}>
							<i className="ri-stack-fill"></i>
						</button>

						<button className={state.showCssCODE ? "active" : ""} onClick={(e) => handleButtonClick("SET_CSS_CODE_VISIBILITY")}>
							CSS
						</button>

						<button className={state.showScssCODE ? "active" : ""} onClick={(e) => handleButtonClick("SET_SCSS_CODE_VISIBILITY")}>
							SASS
						</button>
					</div>
				</div>
				...
				<div className="sidebar__body">
					...
					{state.showColorPalette && state.savedColors.length > 0 && (
						<div className="sidebar--colors-list">
							<ReactSortable list={sortState} setList={setSortState} easing={"cubic-bezier(1, 0, 0, 1)"}>
					...
					{state.showCssCODE && state.savedColors.length > 0 && (
						<div className="sidebar--code sidebar--code--css">
							<SyntaxHighlighter language="css" style={vs} showLineNumbers={false}>
								{`:root {\n${state.savedColors
									.map((color, index) => {
