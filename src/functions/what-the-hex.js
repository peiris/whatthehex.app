...
function findClosestColor(color) {
  ...
  const result = Object.keys(colors).reduce(
    (acc, c) => {
 ...
 */
function generateColorDetails(hexval) {
  let hex = `#${hexval}`;
  if (isValidHexCode(hex)) {
    ...
    let removeSpecialChars = normalize.replace(
      /[\u0300-\u036f-&/\\#,+()$~%.'":*?<>{}]/g,
      ""
    );

    let convertSpaceToHyphen = removeSpecialChars
      .replace(/\s+/g, "-")
      .toLowerCase();
    let convertSpaceToNone = removeSpecialChars
      .replace(/\s+/g, "")
      .toLowerCase();
